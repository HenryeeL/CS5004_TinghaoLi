 ...
 * <p>
 * The "kind" ("Book") is calculated by a method inherited from AbstractBookStoreItem. The other
 * fields must be provided to super when a new Book is constructed. NB: A book can be equal to
 * another book even if their prices are different. Book is a subclass of AbstractBookStoreItem,
 * which implements BookStoreItem.
 */
public class Book extends AbstractBookStoreItem {
   ...
   * <p>
   * The string should be in the following format: Kind: [Book] Title: [title of the book] Author:
   * [first-name last-name] Year: [year of publication] Price: [Price as a decimal number with two
   ...
   * @return true iff the books are equal Different prices do not invalidate equality
   */
  @Override
  ...
  //  FlawFound: Incorrect implementation of equals, currently always return true
  //  FlawFixToDo: Implement proper equality check based on title, author and year
  //  Flaw Fixed: Implemented proper equality check based on title, author and year

  /**
   * Override hashCode since overriding equals
   * <p>
   *
   * @return an int, the hashCode for equal Books Based on title, author, and year
   */
  @Override
  public int hashCode() {
